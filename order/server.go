...
// Проверяем, что вы наш клиент (по ID)
// Смотрим, какие товары вы хотите (ID товаров из корзины)
// Запрашиваем инфу о товарах (название, цена, описание)
// Составляем список "что купили" (товар + количество)
// Сохраняем заказ в базе (фиксируем покупку)
// Показываем вам чек (возвращаем ответ)
...
// Собери все заказы пользователя
// Вытащи все уникальные ID товаров из этих заказов
// Спроси у Catalog Service актуальную инфу об этих товарах
// Собери красивый ответ, подставив названия и описания
func (s *grpcServer) GetOrdersForAccount(
	ctx context.Context,
	r *pb.GetOrdersForAccountRequest,
) (*pb.GetOrdersForAccountResponse, error) {
	acountOrders, err := s.service.GetOrdersForAccount(ctx, r.AccountId)
	if err != nil {
		log.Println(err)
		return nil, err
	}
	productIDMap := map[string]bool{}
	for _, o := range acountOrders {
